...
class BootstrapFormElement extends FormElement implements TranslatorAwareInterface {
    ...
    protected $classMap = array(
        ...
        'Cityware\Bootstrap\Form\Element\StaticElement' => 'formStatic',
    );
    ...
    protected function renderAddOn($aAddOnOptions) {
        ...
        if ($aAddOnOptions instanceof ElementInterface) {
            ...
            throw new InvalidArgumentException(sprintf(
                    'Addon options expects an array or a scalar value, "%s" given', is_object($aAddOnOptions) ? get_class($aAddOnOptions) : gettype($aAddOnOptions)
            ));
        }
        ...
        if (!empty($aAddOnOptions['text'])) {
            if (!is_scalar($aAddOnOptions['text'])) {
                throw new InvalidArgumentException(sprintf(
                        '"text" option expects a scalar value, "%s" given', is_object($aAddOnOptions['text']) ? get_class($aAddOnOptions['text']) : gettype($aAddOnOptions['text'])
                ));
